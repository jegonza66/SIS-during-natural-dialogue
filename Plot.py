...
def plot_cabezas_canales(channel_names, info, sr, sesion, sujeto, Canales_sobrevivientes, Valores_promedio_abs, Display, n_canales, name, Save, Run_graficos_path):
...
def regression_weights(Pesos_totales_sujetos_todos_canales, info, Band, times, sr, Display, 
    ...
    for j in range(Cant_Estimulos):
        ...
        if Autocorrelation_value and times[-1]>0: ax.vlines(decorr_time, ax.get_ylim()[0], ax.get_ylim()[1], linestyle = 'dashed', color = 'red', label = 'Decorrelation time')
        ax.xaxis.label.set_size(13)
        ax.yaxis.label.set_size(13)
        ax.grid()
...
def Matriz_corr(Pesos_totales_sujetos_promedio, Pesos_totales_sujetos_todos_canales, sujeto_total, Display, Save, Run_graficos_path):
...
def Plot_instantes_interes(Pesos_totales_sujetos_todos_canales, info, Band, times, sr, Display_figure_instantes, 
                           Save_figure_instantes, Run_graficos_path, Cant_Estimulos, Stims_Order, stim, Autocorrelation_value = 0.1):
    ...
    for j in range(Cant_Estimulos):
        ...
        if Autocorrelation_value and times[-1]>0: axs[0].vlines(decorr_time, axs[0].get_ylim()[0], axs[0].get_ylim()[1], linestyle = 'dashed', color = 'red', label = 'Decorrelation time')
        ...
        if Save_figure_instantes: 
            ...
            fig.savefig(save_path_graficos + 'Instantes_interes_{}.svg'.format(Stims_Order[j] if Cant_Estimulos > 1 else stim)) 
