...
class Trial_channel:
    ...
    def __init__(
                self, s=21, trial=1, channel=1, Band = 'All',
                 ...
                 Causal_filter = False, drop_bads = False
                 ):
        ...
        self.wav_fname = "Datos/wavs/S"+str(s)+"/s"+str(s)+".objects."+ "{:02d}".format(trial)+".channel"+str(channel)+".wav" 
        self.pitch_fname = "Datos/Pitch/S"+str(s)+"/s"+str(s)+".objects."+ "{:02d}".format(trial)+".channel"+str(channel)+".txt"
    ...
    def f_calculate_pitch(self):
        ...
        pitch_and_intensity.extractPI(os.path.abspath(self.wav_fname), os.path.abspath(output_path), praatEXE, minPitch, maxPitch, self.sampleStep, silenceThreshold)
...
class Sesion:
    def __init__(self, sesion=21, Band = 'All', sr=128, tmin = -0.53, tmax = -0.003, 
                 valores_faltantes_pitch = 0, Causal_filter = False, 
        ...
        while run:           
            try:
                Trial_channel_1 = Trial_channel(s = self.sesion, trial = trial, channel = 1, 
                                                Band = self.Band, sr = self.sr, tmin = self.tmin, 
                                                tmax = self.tmax, 
    ...
    def load_procesed(self):
        ...
        if self.Causal_filter:
            eeg_path = self.procesed_data_path + 'EEG/Causal_Sit_{}_Band_{}(tmin{}_tmax{})/'.format(self.situacion, self.Band, self.tmin, self.tmax) + 'Sesion{}.pkl'.format(self.sesion)
        else:
            eeg_path = self.procesed_data_path + 'EEG/Sit_{}_Band_{}(tmin{}_tmax{})/'.format(self.situacion, self.Band, self.tmin, self.tmax) + 'Sesion{}.pkl'.format(self.sesion)
