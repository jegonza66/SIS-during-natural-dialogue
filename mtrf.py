# -*- coding: utf-8 -*-
"""
Created on Sat Aug  7 15:02:05 2021
...
Save_Pesos_Predicciones_Corr_Rmse_buenas = True
...
times = np.linspace(tmin, tmax, len(delays))

###### Paths ######
procesed_data_path = 'saves/Preprocesed_Data/'

Run_graficos_path = 'gráficos/mTRF/'
Path_it = 'saves/mTRF/Fake_it/'
Path_Pesos_Predicciones_Corr_Rmse = 'saves/mTRF/Corr_Rmse_Pesos_Predicciones/'
...
for sesion in sesiones:
    ...
    # for sujeto, eeg, dstims in zip([1], [eeg_sujeto_1], [dstims_para_sujeto_1]):    
        ...
        coefs = np.zeros((n_splits, n_canales, len(delays)+1))
        scores = np.zeros((n_splits, n_canales))
        ...
        for test_round, (train_val_index, test_index) in enumerate(kf_test.split(eeg)):
            ...
            if Buscar_alpha:
                ...
                best_alphas_total.append(best_alpha)
            ...
            if Fit_Model:
                ...
                rf = ReceptiveField(tmin, tmax, sr, feature_names=['envelope'],
                ...
                speech_train = dstims_train_val[:,0]
                speech_train = speech_train.reshape([speech_train.shape[0],1])
                
                speech_test = dstims_test[:,0]
                speech_test = speech_test.reshape([speech_test.shape[0],1])
                
                rf.fit(speech_train, eeg_train_val)
                predicho = rf.predict(speech_test)
                scores[test_round] = rf.score(speech_test, eeg_test)
                coefs[test_round] = rf.coef_[:, 0, :]
                
                # plt.ion()
                # fig, axs = plt.subplots(2,1)
                
                # axs[0].plot(eeg_test[:,0], label = 'Señal')
                # axs[0].plot(predicho[:,0], label = 'Prediccion')
                # axs[0].set_title('Señal original')
                # axs[0].legend()
                
                # axs[1].plot(predicho[:,0])
            ...
            if Simulate_random_data: 
                ...
                toy_iterations = 10
                psd_rand_correlation = Simulation.simular_iteraciones_mtrf_plot(info,sr, situacion,best_alpha, toy_iterations, sesion, sujeto, test_round, dstims_train_val, eeg_train_val, dstims_test, eeg_test)
                psd_rand_correlations.append(psd_rand_correlation)
            
            if Statistical_test:
                Simulation.simular_iteraciones_mtrf(best_alpha, iteraciones, sesion, sujeto, test_round, dstims_train_val, eeg_train_val, dstims_test, eeg_test, Correlaciones_fake, Errores_fake, Save_iterations, Path_it)
        ...
        if Save_Pesos_Predicciones_Corr_Rmse_buenas:
         ...
         ###### Si no ajusto modelo, cargo Correlaciones y Rmse buenas ######
        if not Fit_Model:
            f = open(Path_Pesos_Predicciones_Corr_Rmse + 'Pesos_buenas_ronda_canal_Sesion{}_Sujeto{}.pkl'.format(sesion, sujeto), 'rb')
            Pesos_ronda_canales = pickle.load(f)
            f.close()
        ...
        Corr_tipicas.append(np.mean(Corr_promedio_abs))
        std_tipicas.append(np.std(Corr_promedio_abs))
...
curva_pesos_totales = Plot.Plot_instantes_interes(Pesos_totales_sujetos_todos_canales, info, times, sr, delays, Display_figure_instantes, Save_figure_instantes, Run_graficos_path)
