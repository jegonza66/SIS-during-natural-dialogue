...
for alpha_num, alpha in enumerate(alphas):
    ...
    curva_pesos_totales = Plot.regression_weights(Pesos_totales_sujetos_todos_canales, info, Band, times, sr,
                                                  Display_figure_instantes, Save_figure_instantes, Run_graficos_path,
...
plt.ion()
fig, axs = plt.subplots(3, 1, sharex=True, figsize = (10,25))
plt.suptitle('Ridge Trace - {} - {}'.format(Band, stim))
plt.xlabel('Ridge Parameter')
plt.xscale('log')

axs[0].set_ylabel('Standarized Coefficents')
axs[0].plot(alphas, Standarized_Betas, 'o--')
...
axs[1].vlines(1e2, Correlaciones.min(), Correlaciones.max(), color='black', linestyle='dashed', linewidth=1.5)
axs[1].errorbar(alphas, Correlaciones, yerr=np.std(Correlaciones), fmt='none', ecolor='black', elinewidth=0.5, capsize=0.5)
axs[1].grid()

axs[2].set_ylabel('Mean Error')
axs[2].plot(alphas, Errores, 'o--')
axs[2].vlines(1e2, Errores.min(), Errores.max(), color='black', linestyle='dashed', linewidth=1.5)
