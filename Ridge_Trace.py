...
for Band in Bands:
    ...
    for stim in Stims:
        ...
        for alpha_num, alpha in enumerate(alphas):
            ...
            curva_pesos_totales = np.array(Plot.regression_weights(Pesos_totales_sujetos_todos_canales, info, Band, times, sr,
                                                          Display_figure_instantes, Save_figure_instantes, Run_graficos_path,
                                                          Cant_Estimulos, Stims, stim))

            Standarized_Betas[alpha_num] = curva_pesos_totales.ravel()
