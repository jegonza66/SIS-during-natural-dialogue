...
class Sesion_class:
    ...
    def load_from_raw(self):
        ...
        Sujeto_1 = {'EEG': pd.DataFrame(), 'Envelope': pd.DataFrame(),
                    'Pitch': pd.DataFrame(), 'PitchDer': pd.DataFrame()}

        Sujeto_2 = {'EEG': pd.DataFrame(), 'Envelope': pd.DataFrame(),
                    'Pitch': pd.DataFrame(), 'PitchDer': pd.DataFrame()}
        ...
        for path in [EEG_path, Envelope_path, Pitch_path, Pitch_der_path, Pitch_mask_path]:
            try:
                os.makedirs(path)
            except:
                pass
...
def Load_Data(sesion, stim, Band, sr, tmin, tmax, procesed_data_path, situacion='Escucha', Causal_filter_EEG=True,
    ...
    possible_stims = ['Envelope', 'Pitch', 'PitchDer', 'PitchMask']
